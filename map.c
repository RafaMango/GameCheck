...
void map_destroy(Map map)
{
  ...
  {
    if (pair->key) free(pair->key);     // Liberar la clave si fue duplicada
    if (pair->value) free(pair->value); // Liberar el valor si fue duplicado
    free(pair);                         // Liberar el par
  }
  free(map);             // Liberar el mapa en sí
}