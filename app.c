...
{
    int cumple = 0;
    if (pc->ram >= juego->ram_rec)
        cumple = 2;
    else if (pc->ram >= juego->ram_min)
        cumple = 1;
    else
        cumple = 0;
...
void agregarJuego(Map *mapa, List *lista)
{
    ...
    fgets(juego->nombre, 100, stdin);
    juego->nombre[strcspn(juego->nombre, "\n")] = 0;
    printf("Ingrese requisitos mínimos (CPU): ");
    fgets(juego->cpu_min, 50, stdin);
    juego->cpu_min[strcspn(juego->cpu_min, "\n")] = 0;
    printf("Ingrese requisitos mínimos (GPU): ");
    fgets(juego->gpu_min, 50, stdin);
    juego->gpu_min[strcspn(juego->gpu_min, "\n")] = 0;
    printf("Ingrese requisitos mínimos (RAM): ");
    scanf("%d", &juego->ram_min);
    limpiarBuffer();
    printf("Ingrese requisitos recomendados (CPU): ");
    fgets(juego->cpu_rec, 50, stdin);
    juego->cpu_rec[strcspn(juego->cpu_rec, "\n")] = 0;
    printf("Ingrese requisitos recomendados (GPU): ");
    fgets(juego->gpu_rec, 50, stdin);
    juego->gpu_rec[strcspn(juego->gpu_rec, "\n")] = 0;
    printf("Ingrese requisitos recomendados (RAM): ");
    scanf("%d", &juego->ram_rec);
    limpiarBuffer();
    list_pushBack(lista, juego);
    map_insert(mapa, strdup(juego->nombre), juego);
    // Guardar en archivo
